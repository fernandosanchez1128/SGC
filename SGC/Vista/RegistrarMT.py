...
( Ui_VistaRegistrarMT, QDialog ) = uic.loadUiType( 'VistaRegistrarMT.ui' )

#[1:Crear 2;Editar 3:Consultar]
class RegistrarMT ( QDialog ):
    ...
    def __init__ ( self, parent = None, modo=1):
        ...
        self.ui = Ui_VistaRegistrarMT()
        self.ui.setupUi( self )
        if(self.tipo==1):
            self.ui.lbModo.setText("Registro")
            self.ui.btRegistrar.setText("Registrar")
            self.ui.btRegistrar.setText("Registrar")
    ...
    def cancelar_clicked(self):
        if(self.tipo==4):
            ...
            self.controldigi.eliminarMT(cedula)
        else:
            self.close()
    ...
    def buscar_clicked(self):
        ...
        try:
            ...
            if(5==6):
                ...
                lt=self.controldigi.consultarMT(ced)
                ...
                if (lt!=None or user!=None):
                    ...
                    QMessageBox.information(self, "Registro", "No hay master-teacher con esa identificacion")
                    self.ui.btRegistrar.setEnabled(False)

        except Exception as q:
            QMessageBox.information(self, "Registro", "Error interno, campos faltantes del master-teacher")
            print str(q)
    ...
    def inscribir_clicked(self):
        ...
        if(self.tipo==1):
            if (self.flag==True):
                ...
                msj=self.controldigi.agregarMT(4,parametros)
                if( msj != "Exito"):
                    QMessageBox.information(self, "Registro", "Error al momento de registrar master-teacher\n"+msj)
                else:
                    QMessageBox.information(self, "Registro", "Registro de master-teacher Exitoso!")
        ...
        if(self.tipo==2):
            ...
            if (self.flag==True):
                ...
                mensaje="Editar Master Teacher con cedula:"+self.cedula
                if (QMessageBox.Yes == QMessageBox(QMessageBox.Information, "Confirmar", mensaje, QMessageBox.Yes|QMessageBox.No).exec_()):
                    ...
                    editmsj=self.controldigi.editarMT(self.cedula,parametros)
                    if(editmsj != "Exito"):
                        QtGui.QMessageBox.warning(self, self.tr("Error en Base de Datos"),
                                      QString.fromUtf8("Error al momento de editar masterteacher\n"+editmsj))

                    else:
                        QMessageBox.information(self, "Registro", "Actualizacion de masterteacher Exitoso!")
                        self.controldigi.cerrarSesion()
                else:
                    print("No...")